...
# https://github.com/Dao-AILab/flash-attention/blob/v0.2.8/flash_attn/flash_attention.py  # noqa: E501
import torch
...
class FlashAttention(nn.Module):
    ...
    def forward(
        ...
        Arguments
        ---------
            qkv: The tensor containing the query, key, and value. (B, S, 3, H, D) if key_padding_mask is None  # noqa: E501
    ...
    def _get_pos_embeddings(self, batch_size: int, input_dims: Tuple[int,
        ...
        if self.cpe_mode:
            if self.training:
                ...
                #     top = (p_rows - i_rows) // 2
                #     pos_embed = pos_embed[..., top:top+i_rows, left:left+i_cols]  # noqa: E501
                # else:
...
class Dinov2LayerScale(nn.Module):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        # Huggingface is absurd and it will rename strings that contain `gamma`, which means that the normal DINO implementation  # noqa: E501
        # of LayerScale won't work with HFHub. So we rename the variable to 'grandma', and support loading checkpoints in either  # noqa: E501
        # format
        ...
        if key_a in state_dict:
            ...
            if strict:
                raise KeyError(
                    f"Couldn't find the key {key_a} nor {key_b} in the state dict!"  # noqa: E501
                )
            else:
...
def _patch_layer_scale(model: VisionTransformer):
    ...
    def replace_ls(old_ls: TIMMLayerScale):
        new_ls = Dinov2LayerScale(
            old_ls.gamma.shape[0], inplace=old_ls.inplace)
        new_ls.load_state_dict(old_ls.state_dict())
        return new_ls

    # Monkey patch: Replace TIMM's LayerScale with our modified DINOv2 one, that uses a param name  # noqa: E501
...
def vit_huge_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ViT-Huge model (ViT-H/16) from original paper (https://arxiv.org/abs/2010.11929)."""  # noqa: E501
    model_args = dict(
        patch_size=16,
        embed_dim=1280,
        depth=32,
        ...
        weight_init='skip')
    if pretrained:
        # There is no pretrained version of ViT-H/16, but we can adapt a ViT-H/14 for this purpose  # noqa: E501
...
class RADIOModelBase(nn.Module):
    ...
    def forward(
        ...
        Args:
            x: Input tensor. Unless `make_preprocessor_external` has been called, then the dynamic range of `x` is expected to be `[0, 1]`,  # noqa: E501
                             otherwise `x` is expected to be mean centered with unit standard deviation.  # noqa: E501
            feature_format: ['NLC', 'NCHW'] - The output format for the features.  # noqa: E501
        """
...
class RADIOConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if version == 'radio_v2.5-h':
            resource = dict(
                url=  # noqa: E251
                'https://huggingface.co/nvidia/RADIO/resolve/main/radio_v2.5-h.pth.tar?download=true',  # noqa: E501
                patch_size=16,
                max_resolution=2048,
                vitdet_num_global=4,
            )
    ...
    def to_dict(self):
        """
        Serializes this instance to a Python dictionary. Override the default [`~PretrainedConfig.to_dict`].  # noqa: E501

        Returns:
            `Dict[str, any]`: Dictionary of all the attributes that make up this configuration instance,  # noqa: E501
        """
