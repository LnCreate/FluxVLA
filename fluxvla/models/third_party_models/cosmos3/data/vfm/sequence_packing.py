...
"""Functions for Cosmos3 sequence packing used by FluxVLA MoT.

This module provides utilities for packing text and image sequences together
with support for dense two-way attention patterns (causal, full, noise).
...
from fluxvla.models.third_party_models.cosmos3.model.vfm.mot.unified_3dmrope_utils import (
    get_3d_mrope_ids_text_tokens,
    get_3d_mrope_ids_vae_tokens,
)
from fluxvla.models.third_party_models.cosmos3.model.vfm.utils.data_and_condition import GenerationDataClean
...
# FluxVLA vendor patch: use the repository logger instead of vendoring
# cosmos-framework's logging shim.
log = initialize_overwatch(__name__)
...
class PackedSequence:
    ...
    # Retained from upstream temporal-causal packing metadata; the current FluxVLA
    # dense two-way path does not read it.
...
def _pack_supertokens_temporal_causal(
    ...
    ``num_action_tokens_per_supertoken=0`` is stamped on the pack for future
    temporal-causal attention implementations.
...
JointSequencePack:    Stores all sub-sequences for all-sequences in a single tensor.
                      It supports looser layouts but is less performant. In this implementation,
                      understanding tokens can be placed in either causal or full-attention sub-sequences.
FactoredSequencePack:
                      Stores causal/undersanding and full/generation sub-sequences as separate tensors.
                      It is more restrictive but is more performant. In this implementation,
                      understanding tokens must be on the causal sub-sequence, and generation tokens
                      must be in the full-attention sub-sequence.
    ...
    if pad_for_cuda_graphs:
        ...
        if is_image_batch:
            if need_causal > MAX_CAUSAL_LEN_IMAGE_BATCH:
                ...
                log.info(f'Growing MAX_CAUSAL_LEN_IMAGE_BATCH to {MAX_CAUSAL_LEN_IMAGE_BATCH}')
            if need_full > MAX_FULL_LEN_IMAGE_BATCH:
                MAX_FULL_LEN_IMAGE_BATCH = need_full
                log.info(f'Growing MAX_FULL_LEN_IMAGE_BATCH to {MAX_FULL_LEN_IMAGE_BATCH}')
            ...
            if need_causal > MAX_CAUSAL_LEN_VIDEO_BATCH:
                ...
                log.info(f'Growing MAX_CAUSAL_LEN_VIDEO_BATCH to {MAX_CAUSAL_LEN_VIDEO_BATCH}')
            if need_full > MAX_FULL_LEN_VIDEO_BATCH:
                MAX_FULL_LEN_VIDEO_BATCH = need_full
                log.info(f'Growing MAX_FULL_LEN_VIDEO_BATCH to {MAX_FULL_LEN_VIDEO_BATCH}')
            causal_seq, full_only_seq = _pad(
                causal_seq,
                full_only_seq,
...
def factored_from_joint_sequence(
    ...
    NOTE: Some arguments seem redundant because they in principle support looser sequence setups.
...
def joint_from_joint_sequence(
    ...
    This keeps the upstream joint layout constructor available for debugging and
    tests. Differently from FactoredSequencePack, it has less strict requirements
