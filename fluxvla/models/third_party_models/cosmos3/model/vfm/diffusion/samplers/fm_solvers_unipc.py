...
class FlowUniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        if use_kerras_sigma:
            # Match diffusers>=0.37 UniPC flow/Karras schedule used by the
            # public Cosmos3 checkpoints. The final sigma is appended below.
            sigma_max = 200.0 if sigma_max is None else float(sigma_max)
            sigma_min = 0.147 if sigma_min is None else float(sigma_min)
